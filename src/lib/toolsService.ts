...
interface FilterCriteria {
  ...
  maxPrice?: number; // monthly per user
}
...
export function filterTools(criteria: FilterCriteria): Tool[] {
  ...
  if (criteria.priceTier) {
    const tiers = Array.isArray(criteria.priceTier) ? criteria.priceTier : [criteria.priceTier];
    filtered = filtered.filter(tool => tiers.includes(tool.priceTier));
  }

  if (criteria.companySize) {
    const sizes = Array.isArray(criteria.companySize) ? criteria.companySize : [criteria.companySize];
  ...
  if (criteria.learningCurve) {
    filtered = filtered.filter(tool => tool.learningCurve === criteria.learningCurve);
  }
...
export function getSimilarTools(toolId: string, limit: number = 5): Tool[] {
  const tool = getToolById(toolId);
  if (!tool) return [];

  // Find tools in same category with similar characteristics
  return toolsData
    .filter(t => 
      t.id !== toolId && 
...
interface UserProfile {
  ...
  role?: string;
  budget?: number; // monthly per user
  teamSize?: number;
  priorities?: ('easy' | 'affordable' | 'powerful' | 'integrations')[];
}

export function recommendTools(profile: UserProfile, category?: ToolCategory): Tool[] {
  ...
  // Score and rank tools based on priorities
...
export function getTopRated(category?: ToolCategory, limit: number = 10): Tool[] {
  let tools = category ? getToolsByCategory(category) : toolsData;
...
export function calculateMonthlyCost(tool: Tool, teamSize: number): string {
  // TODO: Parse pricingDetails and calculate actual cost
